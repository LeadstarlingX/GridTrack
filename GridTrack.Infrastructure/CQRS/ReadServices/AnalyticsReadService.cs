...
public sealed class AnalyticsReadService : IAnalyticsReadService
{
    ...
    public async Task<GetAnalyticsSummaryResponse> GetSummaryAsync(DateTime? from, DateTime? to, CancellationToken ct)
    {
        ...
        var df = allowed is null ? string.Empty : """AND "DistrictId" = ANY(@AllowedDistricts::text[])""";

        var sql = $"""
                   SELECT
                       (SELECT COUNT(*)::int
                        FROM public."Deliveries"
                        WHERE "CreatedAt" >= @From AND "CreatedAt" < @To {df}) AS "TotalDeliveriesToday",

                       (SELECT CASE WHEN COUNT(*) = 0 THEN 0.0
                                    ELSE COUNT(*) FILTER (WHERE "Status" = 4)::float / COUNT(*)::float
                               END
                        FROM public."Deliveries"
                        WHERE "CreatedAt" >= @From AND "CreatedAt" < @To {df}) AS "CompletionRate",

                       (SELECT COUNT(*)::int
                        FROM public."Drivers"
                        WHERE "IsActive" = true {df}) AS "ActiveDrivers",

                       (SELECT CASE WHEN COUNT(*) = 0 THEN 0.0
                                    ELSE COUNT(*) FILTER (WHERE "AnomalyFlag" = true)::float / COUNT(*)::float
                               END
                        FROM public."Deliveries"
                        WHERE "CreatedAt" >= @From AND "CreatedAt" < @To {df}) AS "AnomalyRate",

                       (SELECT COUNT(*)::int
                        FROM public."Deliveries"
                        WHERE "Status" = 0 {df}) AS "PendingDeliveries",
                       ...
                       (SELECT COALESCE(
                        ...
                        WHERE "Status" = 4
                          ...
                          AND "DeliveredAt" >= @From AND "DeliveredAt" < @To {df}) AS "AvgDeliveryMinutes",
                       ...
                       (SELECT CASE
                        ...
                        FROM public."Deliveries"
                        WHERE "Status" = 4
                          AND "DeliveredAt" >= @From AND "DeliveredAt" < @To {df}) AS "OnTimeRatePct",

                       NOW() AS "UpdatedAt"
                   """;

        var row = await connection.QueryFirstAsync<GetAnalyticsSummaryResponse>(
            sql, new { From = rangeFrom, To = rangeTo, AllowedDistricts = allowed });
        return row;
    }
        ...
        CancellationToken ct)
    {
        ...
        var param = new { From = from, To = to, AllowedDistricts = allowed };
        ...
        CancellationToken ct)
    {
        ...
        var items = await connection.QueryAsync<DistrictVolumeItemResponse>(
            sql, new { From = from, To = to, AllowedDistricts = allowed });
        ...
        CancellationToken ct)
    {
        ...
        var rows = (await connection.QueryAsync<PerformanceRow>(
            sql, new { From = from, To = to, AllowedDistricts = allowed })).ToList();
    ...
    public async Task<GetDriverUtilizationResponse> GetDriverUtilizationAsync(int topCount, CancellationToken ct)
    {
        ...
        var allowed  = AllowedDistricts;
        var driverWh = allowed is null ? string.Empty
            : """WHERE d."DistrictId" = ANY(@AllowedDistricts::text[])""";
        ...
        var rows = (await connection.QueryAsync<DriverThroughputRow>(
            sql, new { AllowedDistricts = allowed })).ToList();
    ...
    public async Task<GetDriverAnalyticsResponse> GetDriverAnalyticsAsync(CancellationToken ct)
    {
        ...
        var driverWh   = allowed is null ? string.Empty
            : """WHERE d."DistrictId" = ANY(@AllowedDistricts::text[])""";
        var delivDf    = allowed is null ? string.Empty
        ...
        var p = new { AllowedDistricts = allowed };
        var mainRows    = (await connection.QueryAsync<DriverStatsRow>(mainSql, p)).ToList();
        var districtAvg = (await connection.QueryAsync<DistrictAvgRow>(districtSql, p))
            .ToDictionary(r => r.DistrictId, r => r.AvgDurationSeconds);
        var hourlyRows  = (await connection.QueryAsync<HourlyRow>(hourlySql, p))
        ...
        int hoursAhead, CancellationToken ct)
    {
        ...
        var items = _districtDataService.GetAll()
            .Where(d => allowed is null || allowed.Contains(d.Id, StringComparer.OrdinalIgnoreCase))
